function p = ConnectGaussCompte2D(p,fromid,toid,recept,eff,conProb,plus,minus,sigma)
%function p = ConnectGaussCompte2D(p,fromid,toid,recept,conProb,eff,plus,minus,sigma)
...
if length(plus)<length(to)
    plus=repmat(plus,1,size(to,2));
end

if length(minus)<length(to)
    minus=repmat(minus,1,size(to,2));
end

if length(sigma)<length(to)
    sigma=repmat(sigma,1,size(to,2));
end

if length(conProb)<length(to)
...
for i=1:length(from)
   ...
   p(from(i)).connection(len+1).conntype=4;
