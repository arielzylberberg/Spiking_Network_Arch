...
#if BITS == 63                   
#define MSB          0x4000000000000000L
#define ALL_BITS     0x7fffffffffffffffL
#define HALF_RANGE   0x2000000000000000L
#define STEP         3
#endif
...
void R250::init(const long int sd)
{
	...
	if ( r250_buffer == NULL )	// only do init once
		r250_buffer = new unsigned long int[500];
	...
	r250_index = 0;
	for (j = 0; j < 500; j++)	/* fill r250 buffer with 15 bit values */
#ifdef TRUST_RAND
#if BITS == 32 || BITS == 31 || BITS == 64
	...
	for (j = 0; j < 500; j++)	/* set some MSBs to 1 */
#ifdef TRUST_RAND
		if ( rand() > HALF_RANGE )
			r250_buffer[j] |= MSB;
#else
...
unsigned long int R250::rani()	/* returns a random unsigned integer */
{
	...
	if ( r250_index >= 397 )
		j = r250_index - 397;	/* wrap pointer around */
	else
		j = r250_index + 103;
	...
	if ( r250_index >= 499 )	/* increment pointer for next time */
		r250_index = 0;
	else
		r250_index++;
...
Real R250::gaussian(){
	static Real x1, x2, w, res, y2;

	if (!avail){
		do {
			x1 = 2.0 * this->ranf() - 1.0;
			x2 = 2.0 * this->ranf() - 1.0;
			w = x1 * x1 + x2 * x2;
		} while ( w >= 1.0 );
